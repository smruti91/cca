...
if(isset($_POST) && !empty($_POST['action'])){
  ...
  if($action=="approve"){
    ...
    $result_insert =  generateSQL("UPDATE `cca_team_employee_transaction` SET `rev_status` = ?, `rev_drop_comment` = ?,`rev_add_date`=? WHERE `id` = ? " ,array('Approved',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
  if($action=="reject"){
    $result_insert = generateSQL("UPDATE `cca_team_employee_transaction` SET `rev_status` = ?, `rev_drop_comment` = ?,`rev_add_date`=? WHERE `id` = ? " ,array('Rejected',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
}
...
$get_member = generateSQL("SELECT *  FROM `cca_team_employee_transaction` WHERE `dept_id` =?",array($dept1),false,$mysqli);
$count = 0;
