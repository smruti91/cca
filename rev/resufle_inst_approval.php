...
if(isset($_POST) && !empty($_POST['action'])){
  ...
  if($action=="approve"){
    ...
    $result_insert =  generateSQL("UPDATE `cca_team_institution_transaction` SET `rev_status` = ?, `rev_drop_comment` = ?,`rev_add_date`=? WHERE `id` = ? " ,array('Approved',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
  if($action=="reject"){
    $result_insert = generateSQL("UPDATE `cca_team_institution_transaction` SET `rev_status` = ?, `rev_drop_comment` = ?,`rev_add_date`=? WHERE `id` = ? " ,array('Rejected',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
}
...
$get_member = generateSQL("SELECT *  FROM `cca_team_institution_transaction` WHERE `dept_id` =?",array($dept1),false,$mysqli);
$count = 0;
foreach( $get_member as $row )
{
$count++;
$get_emp_name = generateSQL("SELECT institution_name  FROM `cca_institutions` WHERE  id= ?",array($row['inst_id']),false,$mysqli);
...
<td>
  <?php 
                              if($row['rev_status']!=NULL && $row['rev_status']!='pending'){
                                if($row['rev_status']=="Approved"){
                                ?>
                                <div class="btn btn-success" disabled>Submitted to FA</div>
                                <?php
                              } else if($row['rev_status']=="Rejected"){
                                ?>
...
<script>
 ...
 function manage_team(action,id){
  ...
  $.post("resufle_inst_approval.php",{action:action,id: id,comments: comments},function(res){
    if(res=="success"){
      location.reload();
    }
  });
