    ...
    //echo count($audit_type);
      ...
      if(isset($_POST['save'])) {
      	 ...
      	 for ($i=0; $i <$num ; $i++) { 
      	 	 ...
      	 	 $sql_advance_0 = " INSERT INTO  cca_para_3c (mngplan_id , para_id , advnc_outstd_asOn ,cash_book, amnt_outstd , amnt_paid_atAudit ,amnt_adjusted,balance_asOn ,amnt_outstd_audit,amnt_outstd_cashBook, status , version,add_by,add_ip) values ( '$manageplan_id' , '$para_id' , '$advnc_outstanding_asOn[$i]' , '$cashBook[$i]', '$amut_outStanding[$i]' , '$amut_paid_audit_period[$i]' ,'$amut_adjust[$i]','$balance_asOn[$i]' ,'$amut_outStanding_audit[$i]' ,'$amut_outStanding_cashBook[$i]' , 'draft' , 0 ,'$user_id' ,'$ipaddress' )  ";
             ...
             $last_id = $mysqli->insert_id;
              ...
              if($last_id){
                  $sql_advance_1 = " INSERT INTO  cca_para_3c (mngplan_id , para_id , advnc_outstd_asOn ,cash_book, amnt_outstd , amnt_paid_atAudit ,amnt_adjusted,balance_asOn ,amnt_outstd_audit,amnt_outstd_cashBook, status , version,add_by, tbl_id,add_ip) values ( '$manageplan_id' , '$para_id' , '$advnc_outstanding_asOn[$i]' , '$cashBook[$i]', '$amut_outStanding[$i]' , '$amut_paid_audit_period[$i]' ,'$amut_adjust[$i]','$balance_asOn[$i]' ,'$amut_outStanding_audit[$i]' ,'$amut_outStanding_cashBook[$i]' , 'draft' , 1 ,'$user_id' , '$last_id','$ipaddress' )  ";
