<?php
  ...
  // print_r($_POST); exit;
      ...
      if(isset($_POST['save'])){
      	 ...
      	 for($i=0; $i<$num ; $i++  )
      	 {
      	 	$sql_maintance_0 = " INSERT INTO `cca_para_d` ( `mngplan_id`, `paragraph_id`, `recordList_id`, `status`, `version`, `add_by`, `add_ip`) VALUES ('$manageplan_id', '$para_id', '$record_list_id[$i]', '$checklist[$i]', 0, '$user_id', '$ipaddress') ";
      	 	...
      	 	$sql_maintance_1 = " INSERT INTO `cca_para_d` ( `mngplan_id`, `paragraph_id`, `recordList_id`, `status`, `version`, `add_by`, `tbl_id`, `add_ip`) VALUES ('$manageplan_id', '$para_id', '$record_list_id[$i]', '$checklist[$i]', 1 , '$user_id', '$last_id' , '$ipaddress') ";
      ...
      if(isset($_POST['update'])){
         ...
         for ($i=0; $i < $num ; $i++) { 
             ...
             $update_sql = "UPDATE `cca_para_d` SET `status` = '$checklist[$i]' WHERE `cca_para_d`.`id` = '$record_list_id[$i]';";
             $update_res = mysqli_query($mysqli,$update_sql);

             if($update_res){
                
                 $update_sql1 = "UPDATE `cca_para_d` SET `status` = '$checklist[$i]' WHERE `cca_para_d`.`tbl_id` = '$record_list_id[$i]';";
                ...
                echo "<script> sessionStorage.setItem('message', 'List of Records verified update successfully') ; sessionStorage.setItem('type', 'success') ; window.location = 'record_maintenance.php';</script>";
