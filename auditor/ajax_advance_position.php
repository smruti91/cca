      ...
      if(isset($_POST['update'])) {
          ...
          for ($i=0; $i <$num ; $i++) { 
               ...
               if($para_edit_id[$i] == ''){
                 ...
                 $update_sql = "UPDATE `cca_para_3c1` SET `offical_name` = '$offical_name[$i]', `gpf_Ehrms_id` = '$gpf_Ehrms_id[$i]', `scheme_cash_book` = '$scheme_cashBook[$i]', `amnt_outstd` = '$amut_outStanding[$i]', `amnt_paid_atAudit` = '$amut_paid_audit_period[$i]', `amnt_adjusted` = '$amut_adjust[$i]', `balance_asOn` = '$balance_asOn[$i]', `amnt_outstd_audit` = '$amut_outStanding_audit[$i]', `amnt_outstd_cashBook` = '$amut_outStanding_cashBook[$i]' WHERE `cca_para_3c1`.`id` = '$para_edit_id[$i]'";
                 
                $update  = mysqli_query($mysqli , $update_sql);
               if( $update ){
                   $update_sql1= "UPDATE `cca_para_3c1` SET `offical_name` = '$offical_name[$i]',  `gpf_Ehrms_id` = '$gpf_Ehrms_id[$i]',  `scheme_cash_book` = '$scheme_cashBook[$i]', `amnt_outstd` = '$amut_outStanding[$i]', `amnt_paid_atAudit` = '$amut_paid_audit_period[$i]', `amnt_adjusted` = '$amut_adjust[$i]', `balance_asOn` = '$balance_asOn[$i]', `amnt_outstd_audit` = '$amut_outStanding_audit[$i]', `amnt_outstd_cashBook` = '$amut_outStanding_cashBook[$i]' WHERE `cca_para_3c1`.`tbl_id` = '$para_edit_id[$i]'";
       ...
       if(isset($_POST['action']) =='delete' ){
        ...
        $id = $_POST['edit_id'];
         ...
         $delete_sql_0 = " DELETE FROM `cca_para_3c1` WHERE id='$id' "; 
          $delete_0 = mysqli_query($mysqli, $delete_sql_0);

          if($delete_0){
              $delete_sql_1 = " DELETE FROM `cca_para_3c1` WHERE tbl_id='$id' "; 
