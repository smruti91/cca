...
foreach ($datearry as $dateval) {
 ...
 //print_r($_POST);
  ...
  $org_name=$_POST['org_name_'.$dateval];
  ...
  if($workorder!=""){
      ...
      if(mysqli_num_rows($find_diarydate)>0){
             mysqli_query($mysqli,"update `cca_diary` set objection_memo_issued='$objection_issued', objection_memo_returned='$objection_returned',org_id='$org_name',workorder='$workorder',workdetails='$workdetails',mandays='$mandays',add_date='$add_date',add_ip='$add_ip' where diary_date='".$diary_date."' and emp_id='".$emp_id."'"); 
      }else{
          $result = mysqli_query($mysqli, "INSERT INTO `cca_diary`(`plan_id`,`team_id`,`emp_id`,`diary_date`, `objection_memo_issued`, `objection_memo_returned`,`org_id`, `workorder`, `workdetails`, `mandays`,`diary_status`,`add_date`,`add_ip`) VALUES ('$plan_id','$team_id','$emp_id','$diary_date','$objection_issued','$objection_returned','$org_name','$workorder','$workdetails','$mandays',0,'$add_date','$add_ip')");
      }
  }
}
                  ...
                  <div class="datecontainer">
                    ...
                    $datarray=array();
                      for ($i=0;$i<count($dates);$i++){
                        ...
                        $org_id=$diarydate_result['org_id'];
                        }else{
                       ...
                       <table style="float:left;font-size:14px;width: 48%; margin:1%;" class="table table-bordered drtable">
                        ...
                        <tr>
                          ...
                          $d1=date('Y-m-d',$unixTimestamp);
                           $findmngplan=mysqli_query($mysqli,"select * from `cca_manageplan` where actual_audit_startdate <='".$d1."' and institute_audit_status='Inprogress' and dept_id='".$_SESSION['dept_id']."'");
                          ...
                          <td class="diary_page">
                            <select name="org_name_<?php echo $unixTimestamp; ?>" class="form-control" onchange="change_workdetails(this.value,'<?php echo $unixTimestamp; ?>');">
                              <option value="">--Select One--</option>
                              <option value="office" <?php if($org_id=="office"){echo "selected='selected'";} ?>>Office</option>
                              <?php
                               if(mysqli_num_rows($findmngplan)>0){
                                 while($findmngplan_details=mysqli_fetch_array($findmngplan)){?>
                                  <option value="<?php echo  $findmngplan_details['org_id']; ?>" <?php if($org_id==$findmngplan_details['org_id']){echo "selected='selected'";} ?>><?php echo  $org_name=find_institutionname($findmngplan_details['org_id'],$mysqli); ?></option>
                                <?php  }
                              }else{
                        ...
                        <tr>
                          ...
                          <td class="diary_page">
                              ...
                              ?>
                                 <select style="width:340px" name="workdone_<?php echo $unixTimestamp; ?>" onchange="findworkDetailscnd(this.value,'<?php echo $unixTimestamp; ?>')" class="form-control">
                                    <option value="">--Select one--</option>
                                    <?php 
...
function change_workdetails(ins_type,date){
  if(ins_type=='office'){
    // $("#ins_type")
    console.log(date);
   
    $('select[name="workdone_'+date+'"]').val('74');
    findworkDetailscnd('74',date);
  }
}
</script>
