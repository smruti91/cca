  ...
  //print_r($_POST); exit;
   ...
   if( isset($_POST['save_irreg']) )
   {
       ...
       // $msg = array();
        for($i = 0;$i<$num;$i++  )
        {
           $sql_irreg_0 = " INSERT INTO  cca_persistent_irreg (managePlan_id , paragraph_id , irreg_notice, status , trans_no) values ( '$manageplan_id' , '$para_id' , '$irreg_notice[$i]' , 'draft' , 0)  ";
          // echo $sql_complaince;
           $insert = mysqli_query($mysqli , $sql_irreg_0);
           $last_id = $mysqli->insert_id;
           //echo $last_id;
            if($last_id){
                $sql_irreg_1 = " INSERT INTO  cca_persistent_irreg (managePlan_id , paragraph_id , irreg_notice , status , trans_no,irreg_id) values ( '$manageplan_id' , '$para_id' , '$irreg_notice[$i]' , 'draft' , 1 ,'$last_id' )  ";
   ...
   // update complaince details
    ...
    if( isset($_POST['Update_irreg']) ){
          ...
          $num = count($irreg_notice);
            ...
            for($i = 0;$i<$num;$i++  ){
               ...
               $update_sql= "UPDATE `cca_persistent_irreg` SET  `irreg_notice` = '$irreg_notice[$i]' WHERE CONCAT(`cca_persistent_irreg`.`id`) = '$irreg_edit_id[$i]' ";
              
                $update  = mysqli_query($mysqli , $update_sql);
               if( $update ){
                   $update_sql1= "UPDATE `cca_persistent_irreg` SET  `irreg_notice` = '$irreg_notice[$i]' WHERE irreg_id = '$irreg_edit_id[$i]' ";
    ...
    if(isset($_POST['action']) =='delete' ){
        ...
        $id = $_POST['edit_id'];
         ...
         $delete_sql_0 = " DELETE FROM `cca_persistent_irreg` WHERE id='$id' "; 
          $delete_0 = mysqli_query($mysqli, $delete_sql_0);

          if($delete_0){
              $delete_sql_1 = " DELETE FROM `cca_persistent_irreg` WHERE irreg_id='$id' "; 
