                    ...
                    <?php
                       ...
                       while($res_rowsubhead=mysqli_fetch_array($result_subhead)){ ?>
                          ...
                          <tr>
                            ...
                            <?php
							 if($res_rowsubhead['id'] == 9){
                                   $table = 'cca_prev_audit_obs';
                                }
                                if($res_rowsubhead['id'] == 10){
                                   $table = 'cca_persistent_irreg';
                                }
                                
                          $sql_para  = " SELECT * FROM ".$table." WHERE paragraph_id = '".$res_rowsubhead['id']."'  AND managePlan_id = '".$manageplan_id."' " ; 
                          //echo $sql_para;
                          ...
                          //print_r($para_row);
                              if($para_row != ''){
                                ?>
                                 <td><a href="javascript: ccadatapost('<?php echo $res_rowsubhead['page_url'];?>',{para_id:'<?php echo $para_row['paragraph_id'];?>' , mangPlan_id: '<?php echo $para_row['managePlan_id']  ; ?>' , edit_id: 1 });" class="btn btn-info">Edit</a></td>
                                <?php
                              }else{
                                ?>
