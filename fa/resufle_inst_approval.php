...
if(isset($_POST) && !empty($_POST['action'])){
  ...
  if($action=="approve"){
    ...
    $result_insert =  generateSQL("UPDATE `cca_team_institution_transaction` SET `hod_status` = ?, `hod_comment` = ?,`hod_appr_dis_date`=? WHERE `id` = ? " ,array('Approved',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
  if($action=="reject"){
    $result_insert = generateSQL("UPDATE `cca_team_institution_transaction` SET `hod_status` = ?, `hod_comment` = ?,`hod_appr_dis_date`=? WHERE `id` = ? " ,array('Rejected',$comments,$date,$id),true,$mysqli);
    echo "success";
    exit();
  }
}
...
$get_member = generateSQL("SELECT *  FROM `cca_team_institution_transaction` WHERE `dept_id` =? AND `rev_status`=?",array($dept1,'Approved'),false,$mysqli);
$count = 0;
